...
trigger_error(mysqli_connect_error());
	...
	$sql2 = "SELECT id_risco FROM risco WHERE desc_risco = '$risco'";
	mysqli_query($conn, $sql2);  

	$sql3 = "SELECT id_classe FROM classe WHERE desc_classe = '$classe'";
	mysqli_query($conn, $sql3);  

	$query = $conn->query("INSERT INTO lab.reagente (`lote`,`cas`,`desc_reag`,`local_reag`,`controlado`,`area_reag`,`id_classe`,`id_risco`)  VALUES (\"$lote\", \"$cas\", \"$desc_reag\", \"$local_reag\", \"$controlado\", \"$area_reag\", \"$classe\", \"$risco\");");

	$query = $conn->query("INSERT INTO lab.Estoquereag (`qtd_reag`,`unidade`,`validade`,`desc_reag`,`cas`,`lote`)  VALUES (\"$qtd_reag\", \"$unidade\", \"$validade\",\"$desc_reag\",\"$cas\",\"$lote\");");
	try{
		$conn->close();
		try {
			$conn->close();
			...
			echo false;
			$sql3 = "DELETE FROM reagente WHERE cas = '$cas'";
			$conn->close();
		}
	}catch(Exception $e){
		echo false;
...
<?php
	...
	$cons = "SELECT id_risco FROM lab.risco WHERE desc_risco = '$risco'";
    $res = $con->query($cons);
	...
	$sql3 = "SELECT id_classe FROM lab.classe WHERE desc_classe = '$classe'";
    $res3 = $con->query($sql3);
