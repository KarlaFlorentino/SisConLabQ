...
<div id="portal-column-content" class="cell width-3:4 position-1:4">
	...
	<div>
		...
		<?php
			...
			$sql = $pdo->prepare("SELECT count(*) AS qtd from lab.reagente WHERE validade < '$dataAtual'");
		    $result = $sql->execute();
		 	$vencido = $sql->fetch(PDO::FETCH_ASSOC);
	 	?>
		<p>- <?php echo $vencido['qtd'];?> Reagente(s) vencido(s).</p>
		<?php  
			$sql = $pdo->prepare("SELECT count(*) AS qtd from lab.reagente WHERE validade - '$dataAtual' > 10");
		    $result = $sql->execute();
		 	$ok = $sql->fetch(PDO::FETCH_ASSOC);
		?>
		<p>- <?php echo $ok['qtd'];?> Reagente(s) OK.</p>
		<?php  
			$sql = $pdo->prepare("SELECT count(*) AS qtd from lab.reagente WHERE validade - '$dataAtual'> 0 AND validade - '$dataAtual' < 11");
		    $result = $sql->execute();
